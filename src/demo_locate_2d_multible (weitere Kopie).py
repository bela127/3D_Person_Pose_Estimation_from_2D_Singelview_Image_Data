...
class Locator(tf.keras.Model):
    ...
    def __init__(self, min_loc, max_loc, bins, window=[3,3]):
        super().__init__()
...
class DiscretLocation(tf.keras.layers.Layer):
    def __init__(self, bins=[10,10]):
        ...
        self.representation = self.add_weight(trainable=True, shape = [1,*self.bins.numpy(),1], initializer = tf.initializers.glorot_uniform())

    def __call__(self):
        return tf.abs(self.representation)
...
class DiscretToContinuousLocation(tf.keras.layers.Layer):
    ...
    def build(self, inputs_shape):
        ...
        self.occurrence_patches = SlidingWindowPatches(self.window, self.stride, padding='SAME')
    ...
    def call(self, discret_loc):
        ...
        center_probability = occurrence_patches[:,:,:,:,self.window_center_index]
        pos_marker1 = tf.nn.sigmoid((center_probability*self.weight1-self.bias1))
        pos_marker2 = tf.nn.sigmoid((center_probability*self.weight2-self.bias2))
