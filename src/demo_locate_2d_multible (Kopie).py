...
def main():
    ...
    def train_loop(epochs):
        ...
        goals_raw = np.array([[[5,5]]], dtype=np.float32)
        ...
        for epoch in range(epochs):
            ...
            goals = gt(clm(goals_raw))
            goals = tf.reshape(goals,goals.shape[:-1])
...
class LocationMap(tf.keras.layers.Layer):
    def __init__(self, min_loc = [0,0], max_loc = [10,10], bins = [10,10]):
        ...
        loc_map = np.meshgrid(*[np.linspace(_min_loc, _max_loc, _num) for _min_loc, _max_loc, _num in  zip(self.min_loc, self.max_loc, self.bins)])
...
class Locator(tf.keras.Model):
    ...
    def __init__(self, min_loc, max_loc, bins):
        ...
        self.dtcl = DiscretToContinuousLocation(stride=[3,3], min_loc = min_loc, max_loc = max_loc, window=[5,5])
...
class ContinuousLocationMap(tf.keras.layers.Layer):
    ...
    def __init__(self, min_loc=[0,0], max_loc=[10,10], bins=[10,10]):
        ...
        self.corr_repr_base = tf.fill([1,*self.bins,2],1.0)
